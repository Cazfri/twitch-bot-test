...
func main() {
	...
	// consumer group), then returns a handle object that holds the key and provides access to the
	// channel. Client then must defer handle.Close() to close out the handle, which closes the
